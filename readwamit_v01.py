...
class readWamit(object):
    ...
    def ReadOutFile(self, wamOUT):	 
        ...
        # essentially the same but there will be different files depending on the 
        # WAMIT run setup.
        #
    ...
    def ReadMode(self, outmode, waveT, nmodes, dat_IJ_AB):
        ...
        # setup/init
        AddedMassHeave =  self.AddedMassHeave
        DampingHeave = self.DampingHeave
        ...
        for i in range(len(waveT)):
            for j in range(nmodes):
                ...
                if dat_IJ_AB[i][j].split()[0:2] == outmode:
                    ...
                    AddedMassHeave.append( dat_IJ_AB[i][j].split()[-2] )
                    DampingHeave.append( dat_IJ_AB[i][j].split()[-1] )
        
        return AddedMassHeave, DampingHeave
